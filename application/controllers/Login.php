...
class Login extends CI_Controller
{
	public function index()
	{
		...
		{
			//The form_validation validates the form before login_check and loging in succesfully


			//display error with form_validation
			$this->form_validation->set_error_delimiters('<div class="error">', '</div>');

			
			//On successfull validation callback login_check method in the server internally
			//login_check have access to post input values
			//if login_check is successfull (after using employee->login sending to it the username and password) it returns true 
			//login_check  also sets the user session with the required data (in employee->login)
			//then $this->form_validation->run() returns true and you are directed to "home"
